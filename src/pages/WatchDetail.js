    ...
    <div className="page" style={{ background: '#ffffff' }}>
      ...
      <div className="detail-layout">
        ...
        <div className="detail-left">
          ...
          {/* Thumbnails — show strip whenever there are images, or when editing (for + button) */}
          {images.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
              {images.map((img, i) => (
                <div
                  key={img.url}
                  draggable={editing}
                  onDragStart={e => { if (!editing) { e.preventDefault(); return; } setDragIndex(i) }}
                  onDragOver={e => { if (editing) e.preventDefault() }}
                  onDrop={() => { if (editing) { handleReorderImages(dragIndex, i); setDragIndex(null) } }}
              ...
              {(() => {
                ...
                if (watch.notes) {
                  ...
                  for (const part of watch.notes.split(' | ')) {
                    ...
                    else if (part.startsWith('Color: ')) displayColor = part.slice(7)
                ...
                return (
                  <div className="detail-meta" style={{ marginBottom: 24 }}>
                    ...
                    {displayGender && (
                      <div className="detail-meta-row">
                        <span className="detail-meta-key"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="4"/><path d="M12 12v9M9 18h6"/></svg>Gender</span>
                        <span>{displayGender}</span>
                      </div>
                    )}
                    {displayColor && (
                      <div className="detail-meta-row">
                        <span className="detail-meta-key"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="13.5" cy="6.5" r="2.5"/><circle cx="17.5" cy="10.5" r="2.5"/><circle cx="8.5" cy="7.5" r="2.5"/><circle cx="6.5" cy="12.5" r="2.5"/><path d="M12 22C6.5 22 2 17.5 2 12s4.5-10 10-10c.926 0 1.648 1.175 1.025 1.887L12 5c-.539.617-.1 1.5.642 1.5H16c1.1 0 2 .9 2 2v.5"/></svg>Color</span>
