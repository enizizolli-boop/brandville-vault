...
export default function AgentListings() {
  ...
  const { rate: usdToCnyRate, refetch: refetchUsdCny } = useExchangeRate('USD', 'CNY')
  ...
  async function handleSaveRate(from, to, value) {
    ...
    try {
      ...
      if (from === 'USD' && to === 'CNY') refetchUsdCny()
  ...
  async function approveSupplierListing(listing) {
    ...
    if (cur === 'USD') {
      ...
      priceEur = (usdToCnyRate && rate) ? Math.round(num / usdToCnyRate / rate) : num
    ...
    if (askingAmt) {
      ...
      else if (askingCur === 'CNY' && usdToCnyRate && rate) costEur = Math.round(askingAmt / usdToCnyRate / rate)
              ...
              <div className="form-row">
                ...
                {bagCostCurrency === 'CNY' && bagCostPrice && (
                  <div style={{ fontSize: 12, color: '#b0a898', marginTop: 4 }}>
                    {(usdToCnyRate && rate) ? `≈ €${Math.round(Number(bagCostPrice) / usdToCnyRate / rate).toLocaleString()}` : 'Loading CNY rate…'}
                  </div>
                )}
              </div>
                ...
                {supEditId === listing.id ? (
                  ...
                  <div style={{ marginBottom: 12 }}>
                    ...
                    {listing.asking_price && (() => {
                      ...
                      const eurEq = cur === 'CNY' && usdToCnyRate && rate
                        ? ` (≈ €${Math.round(listing.asking_price / usdToCnyRate / rate).toLocaleString()})`
                  ...
                  <div style={{ flexShrink: 0 }}>
                    <div style={{ fontSize: 11, color: 'var(--faint)', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
                      ...
                      {(() => {
                        ...
                        const eurHint = ac === 'CNY' && usdToCnyRate && rate ? Math.round(listing.asking_price / usdToCnyRate / rate)
